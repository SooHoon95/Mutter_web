...
- **음악 = SoundCloud 단일(paste-URL + oEmbed 검증).** ~~CC0 무드 픽커/폴백~~은 2026-07-03 사용자 결정으로 제거(무음 허용) — iOS 반영 완료, 웹은 마이그레이션 예정. SC 재생 불가 시 폴백 없이 "음악 없음"으로 표시한다.
...
- **음악 = SoundCloud 단일 / 무음 허용** — (2026-07-03 결정, 구 "무음 편지 0" 폐기) 음악은 SC paste-URL 하나뿐이고 선택 사항이다. 붙이는 시점 oEmbed 검증으로 죽은 링크를 거르고, 재생 불가 시 폴백 없이 "음악 없음"으로 정직하게 표시한다. 광고·끊김이 친밀한 순간을 역전시키지 않게 한다.
