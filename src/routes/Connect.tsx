...
// iOS 앱 설치/핸드오프 CTA는 ConnectHandoff가 단독으로 소유한다(이 페이지의 배너·디바이스 판별 제거).
// 여기서는 로그인된 사용자가 웹에서 초대를 조회·수락하는 흐름만 다룬다.
...
function renderInviteState(
  ...
  // 0. 이미 수락된 초대(앱 또는 다른 곳에서 수락 완료) — 에러가 아닌 성공-등가 안내.
  //    핸드오프: 앱에서 수락 후 웹 폴백으로 복귀한 경우. 토큰은 재소비하지 않는다.
  if (invite.alreadyAccepted) {
    return (
      <>
        <h1 className={styles.heading}>이미 수락된 초대예요</h1>
        <p className={styles.desc}>
          이 초대는 이미 수락됐어요. 주고받은 사람에서 연결을 확인할 수 있어요.
        </p>
