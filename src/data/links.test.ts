...
//   - claim-and-bind (openByToken이 deviceId를 RPC에 전달)
// 보안 수정 검증:
...
import { issueLink, listLinks, revokeLink, openByToken } from './links';
...
describe('issueLink', () => {
  ...
  it('암호 없이 발급하면 hasPassword = false', async () => {
    const rowNoPass = { ...sampleRow, password_hash: null };
    mockGetSupabase.mockReturnValue(
      makeSupabaseMock({
...
describe('M-3: revokedAt 매핑', () => {
  it('revoked=true이면 revoked_at 컬럼 값을 revokedAt으로 반환한다', async () => {
    const revokedRow = {
      ...
      claimed_at: '2026-06-15T10:00:00Z', // 이전에는 이 값을 잘못 사용했음
    };
    mockGetSupabase.mockReturnValue(
      makeSupabaseMock({
...
describe('openByToken', () => {
  ...
  it('RPC get_letter_by_token을 호출하고 편지 페이로드를 반환한다', async () => {
    ...
    const result = await openByToken('tok', 'pass', 'device-001');
    expect(result.id).toBe('letter-abc');
    expect(result.title).toBe('테스트 편지');
    expect(result.templateId).toBe('default');
  });

  it('deviceId와 password를 RPC에 전달한다 (claim-and-bind + 암호 서버 비교 설계 확인)', async () => {
    const sbMock = makeSupabaseMock({
      rpcResult: { data: letterPayload, error: null },
    });
    mockGetSupabase.mockReturnValue(
    ...
    await openByToken('my_token', 'my_password', 'device-abc');
  ...
  it('암호 없이 열 때 p_password는 null로 전달한다', async () => {
    ...
    await openByToken('tok', undefined, 'device-001');
  ...
  it('데이터가 null이면 "편지를 찾을 수 없습니다" 오류를 던진다', async () => {
    ...
    await expect(openByToken('tok', undefined, 'dev')).rejects.toThrow('편지를 찾을 수 없습니다');
  });
  ...
  it('L-3: LINK_REVOKED 에러를 사용자 메시지로 정규화한다', async () => {
    ...
    await expect(openByToken('tok', 'pass', 'dev')).rejects.toThrow(
  ...
  it('L-3: WRONG_PASSWORD 에러를 사용자 메시지로 정규화한다', async () => {
    ...
    await expect(openByToken('tok', 'wrong', 'dev')).rejects.toThrow(
      '암호가 올바르지 않습니다.',
    );
  });

  it('L-3: DEVICE_MISMATCH 에러를 사용자 메시지로 정규화한다', async () => {
    mockGetSupabase.mockReturnValue(
      makeSupabaseMock({
        rpcResult: { data: null, error: new Error('DEVICE_MISMATCH') },
      }) as unknown as ReturnType<typeof getSupabase>,
    );

    await expect(openByToken('tok', 'pass', 'other-device')).rejects.toThrow(
      '다른 기기에서 이미 열린 링크입니다.',
    );
  });
  ...
  it('L-3: TOKEN_NOT_FOUND 에러를 사용자 메시지로 정규화한다', async () => {
    ...
    await expect(openByToken('invalid', undefined, 'dev')).rejects.toThrow(
      '링크를 찾을 수 없습니다.',
    );
  });
  ...
  it('L-3: 알 수 없는 에러는 내부 코드 비노출 메시지로 정규화한다', async () => {
    ...
    await expect(openByToken('tok', undefined, 'dev')).rejects.toThrow(
      '편지를 불러올 수 없습니다.',
    );
  });
});
