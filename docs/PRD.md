# PRD: 감성 편지 앱 (MVP) — v6, SC 단일음원·무음 허용 + N:N 연결 (2026-07-08)

## 0. 문서 상태 / 변경 이력
- v2: 손글씨 합성이 v1 핵심 → v3: 손글씨 v2 연기, "스크롤-동기 음악" v1 핵심 → v4: 음악 소스 확정(SC+CC0 2기둥) → v5: 음악 연출 단순화(단일 트랙) + 계정·소셜·전달 기능 실제 구현 → **v6: CC0/무료음악 기둥 제거(SC 단일·무음 허용) + 연결 독점 1:1 → N:N.**
- **v6 (2026-07-08):** 아래 2가지 반영.
  - ① **CC0/RF 무드 픽커(기둥 3) 완전 제거 → 음악 = SoundCloud 단일 음원, 무음 허용.** 구 원칙 "무음 편지 0"·"CC0 보장형 폴백"·"2기둥"을 폐기. 음악은 SC paste-URL 하나뿐이며 **선택 사항**. 붙이는 시점 oEmbed 검증으로 죽은 링크만 거르고, 수신 시 SC 재생 불가면 **폴백 없이 "음악 없음"으로 정직하게 표시**(무음 허용). (구현: `MoodPicker`/CC0 카탈로그 선택 경로 비활성·고아화, SC 실패 시 CC0 재폴백 없음. 이하 본문의 "CC0"·"무음0"·"카탈로그 폴백" 언급은 모두 무효.)
  - ② **연결 독점 1:1 → N:N(다대다).** 한 사람이 여러 명과 동시에 연결 가능. accept의 배타성 가드 제거, 같은 상대와의 중복 페어만 방지. `disconnect(p_other_user)`로 특정 연결만 해제. (마이그레이션 `0027_connections_n_to_n.sql`.)
...
## 1. 한 줄 목표 (v1)
발신자가 **테마(편지지)에 (선택적으로) SC 음악 1곡을 붙여** 편지를 띄우고, 받는 사람이 **설치 없이 링크로 열어("편지 열기 ▶")** 음악과 함께(없으면 무음) 편지를 읽는 모바일 웹/PWA. 계정이 있으면 받은편지 보관·답장·상대별 주고받은 편지 모아보기까지.

### 핵심 가치 루프 (v1)
작성(**테마 + 본문 + 음악(선택) 1곡**: SC 곡 붙여넣기, 없으면 무음) → 무설치 링크 발급(암호 기본 ON) →
수신자 **"편지 열기 ▶"** 탭 → **음악 있으면 자동재생**(없으면 무음) + 편지 읽기 → (계정 시) 받은편지함 보관·답장.

## 2. 원칙
1. **연출되는 편지가 본질.** "동기화 음악"은 보이지 않는다. 카카오 카드가 못 하는 *발신자의 연출*을 판다.
2. **음악 = SoundCloud 단일 / 무음 허용.** (구 "무음 편지 0" 폐기) 음악은 SC paste-URL 하나뿐이고 선택 사항이다. 재생 불가 시 폴백 없이 "음악 없음"으로 정직하게 표시한다.
...
| 항목 | 확정값 | 근거 |
| --- | --- | --- |
| v1 음악 | **SoundCloud 단일 음원(선택), 무음 허용** (구 "+CC0 2기둥" 폐기) | PoC 검증 + 단순함·무음 정직성 |
| 작성자 업로드 | **v2로 연기(out)** | $0에서 DMCA/OSP 면책 기계장치 선구축 필요 |
| 음악 연출 | **편지당 1곡 자동재생**("열기 ▶" 직후). ~~스크롤 동기/단락별 큐~~ 폐기(v5) | 수신자 싱크 미인지(PRE-MORTEM #3) + 단순함 |
| 음악 추상화 | 단일 `TrackSource`(SoundCloudSource 주 사용; HostedAudioSource 구현은 잔존하나 선택 경로 비활성), 재생/일시정지 | 소스 무관 추상화 유지 |
...
| 연결(관계) | **N:N(다대다)** — 한 사람이 여러 명과 연결(구 "독점 1:1" 폐기). 초대 링크·직접발송·상대별 스레드 | 관계 그래프 확장 |
...
## 4. 음악 아키텍처 (v6: SC 단일·무음 허용)
**Decision (v6):** 음악은 **SoundCloud paste-URL 단일 소스**이며 **선택 사항**이다. 구 v4의 "CC0/RF 무드 픽커 보장형 fallback(2기둥)"은 **완전 폐기** — 발신자가 SC 곡을 안 붙이면 그냥 **무음**이고, 붙인 SC가 재생 불가(삭제·geo·embed-disabled)면 **폴백 없이 "음악 없음"으로 표시**한다(무음 허용). `TrackSource` 추상화와 `HostedAudioSource` 구현은 코드에 잔존하나 선택 경로(무드 픽커)는 비활성·고아. **Creator Upload는 여전히 v2**(면책 기계장치 선구축 필요).

| Pillar | 언제 | 합법성 | 한계 |
|---|---|---|---|
| **SoundCloud embed** (Widget API) | 발신자가 SC URL로 '우리 노래' 지정. **v1 유일 음원** | OK — 공식 Widget API/canonical embed만. *오디오 바이트 fetch/저장/proxy 순간 면책 상실* | 검색 불가(paste-URL만), 광고 가능, 삭제 시 dead→무음, embed-disabled/geo 실패, 한국 대중가요 빈약 |
| ~~CC0/RF 카탈로그~~ | ~~보장형 fallback~~ | — | **v6에서 제거.** 무음 허용으로 대체(보장형 음악 없음) |
| **Creator Upload** | 발신자 파일 업로드 | 조건부 — OSP 면책 *절차 이행* 필수 | **v2**. |

## 5. 검증 상태 & 빌드 순서
- **SoundCloud PoC ✅ 검증 완료** (iOS Safari: "열기 ▶" 제스처 후 추가 탭 없이 스크롤 `seekTo` 동작, 순수 오디오, 무로그인 풀재생). 산출물: `poc/spike-music-soundcloud.html`.
- **빌드 순서(이력):** ① `HostedAudioSource` + CC0 카탈로그 먼저(싱크 엔진 디리스크) → ② `SoundCloudSource`(검증됨) → ③ (v2) Upload. **(v6: CC0 카탈로그·무드 픽커는 이후 제거 — SC 단일·무음 허용.)**

## 6. 수용기준 (AC)
- embed-disabled/private/geo/비200 SC URL은 **발신자 작성 시점에 거부**되고 수신자에게 도달 안 함(즉시 fallback 안내).
- 발신자가 SC 곡을 안 붙이면 편지는 **무음**이다(무음 허용 — 구 "무음 편지 0" 폐기).
- 수신 로드 시 SC liveness 실패하면 **폴백 없이 "음악 없음"으로 표시**(구 "카탈로그 폴백" 폐기). 본문은 항상 보존.
- ~~모든 CC-BY 트랙 크레딧 렌더~~ / ~~ingestion 라이선스 게이트·프로비넌스~~ — **v6에서 제거**(CC0 카탈로그 없음. v2 Upload 시 재도입).
...
| US-003 | 편지 작문 + 음악 1곡 선택 | arch | 테마+본문 WYSIWYG, SC paste-URL **oEmbed 검증**(불가 트랙 거부)로 **1곡(선택)** 지정, 없으면 무음 |
| US-004 | 음악 재생기(`TrackSource`) | integration | SoundCloudSource+HostedAudioSource 단일 인터페이스, 열기▶ 언락 후 **1곡 자동재생/일시정지**(v5: 단락 seekTo 폐기) |
| ~~US-005~~ | ~~CC0/RF 카탈로그 + 라이선스 게이트~~ | — | **v6 제거** — SC 단일·무음 허용. 라이선스 게이트·프로비넌스는 (v2) Upload 시 재도입 |
| US-006 | 템플릿/타이포 | arch | 5~7 템플릿, 모바일 페이지네이션 |
| US-007 | 전달 링크 + 프라이버시 | infra | ≥128bit, 암호 기본 ON, claim-and-bind, noindex |
| US-008 | 수신자 무설치 웹뷰 | arch | 열기▶→**음악 있으면 자동재생**, **SC liveness 실패→"음악 없음"(무음 허용)**, <3s/4G |
...
## 8. 프라이버시·안전 & 필수 법적 안전장치 (v6: v1 = SC 단일)
- **SC 가드레일:** 공식 Widget API + canonical embed URL만. 스트림 URL rip·proxy·캐시·재호스팅 **금지**(면책 상실). embed-disabled/private 우회 금지 → 실패 시 graceful "음악 없음". SC 브랜딩/컨트롤 은폐·제휴 암시 금지.
- ~~CC0/RF 라이선스 화이트리스트·NC/ND 하드밴·CC-BY 크레딧·프로비넌스·한국 인접권 클리어~~ — **v6에서 제거**(CC0 카탈로그 없음). *(v2) Creator Upload를 켤 때 라이선스 게이트·프로비넌스·인접권 클리어를 재도입한다.*
...
2. **무음 편지(SC 삭제/takedown).** 키프세이크의 음악이 한참 뒤 소급 소실. **v6 수용:** 보장형 CC0 폴백을 제거하고 **무음 허용**으로 전환 — 음악은 선택적 보너스이며, 재생 불가 시 "음악 없음"으로 정직하게 표시(편지 본문은 항상 보존). 구 "절대 무음 아님" 약속은 폐기.
...
## 10. 테스트 플랜
- **Unit:** 단일 트랙 cue 선택 모델, 토큰 엔트로피(≥128bit), oEmbed 검증, expiry/revoke, 닉네임 게이트, 열람기록. ~~라이선스 화이트리스트~~(v6 제거).
- **Integration:** `TrackSource` 추상화, 열기▶ 언락→1곡 자동재생, SC liveness 실패→무음("음악 없음").
- **E2E:** happy(작성→링크→오픈→1곡 재생) + 불행(광고 트랙 경고, embed-disabled 거부, SC 삭제→무음, revoked 링크, 예약공개 전 차단).
...
## 14. 비용 ($0)
- SC=음원비 0, 무음=비용 0, R2=무료 티어(egress 무료, v2 업로드용). 유료 전환은 규모 후(라이선스 음원, 손글씨 GPU, 핑거프린팅).
...
| 작성 | 테마+본문+**SC 음악 1곡(선택)**, **이어쓰기(임시저장)** | 본문 빈 줄로 단락 분리, cue는 첫 단락 1개. `/create/:id` 복귀 |
...
| 관계 | **N:N 연결**(초대 `/connect`, 여러 명과 연결), **직접발송**, **특정 연결 해제**(편지 보존), **상대별 스레드**(`/people`) | 구 "독점 1:1" → v6 N:N. accept 시 advisory lock으로 중복 페어 방지(마이그레이션 0027) |
